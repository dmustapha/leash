...
| - | **[SKILL] REFRAME deepener: registry-RESOLUTION binding (the ENS name IS the leash).** At register, LEASH RESOLVES the ERC-8004 owner on-chain via the canonical Identity Registry (`ownerOf`/`getAgentWallet`) and writes the on-chain-resolved binding into the resolver as `agent.erc8004` (CAIP-10) + `agent.address` text records — so the ENS name binds an existing external identity to the spending account. The binding is RESOLVED live, never hard-coded (the ENS bounty forbids hard-coded ENS values); this makes the ENSv2 name the load-bearing "leash" tying identity → policy. Advisory (INVARIANT #13); labeled "on-chain-resolved", not "verified" (REF-5). | ARCHITECTURE §5 (erc8004 resolve + text-record write); F-029; REFRAME-SCOPE R1/R2; INVARIANT #13 | ✅ deep (REFRAME) |
...
**[SKILL] REFRAME deepener: native Hedera threshold keys + co-signed x402 settle.** The reframe makes the Hedera integration DEEPER by using a genuine Hedera-native primitive: the agent's spending account is a `KeyList[agentPub, leashCoSignerPub]` with `threshold=2` (a native Hedera threshold/multi-key account, not a generic EVM multisig contract or an ERC pattern). The agent signs 1-of-2 client-side; LEASH applies its co-signature at settle ONLY after the policy gate passes, so the x402 settle is co-signed on the Hedera rail. This is a Hedera-native account-security feature (KeyList threshold accounts) doing real work in the payment flow — control = TRUE, but facilitator-trusted, NOT chain-enforced trustlessness (REF-3; never claim the chain enforces the cap). Evidence: F-030 (agent-alone ⇒ MISSING_COSIGN), F-031 (operator/LEASH-alone can't move funds), REFRAME-SCOPE S1/S2.
...
**[SKILL] REFRAME note: Privy funds the co-signed spending account at its long-zero EVM address; DENY beat preserved.** Under the reframe the spending account is a Hedera `KeyList` threshold-2 account (no key-derived EVM alias, REF-2), so Privy funds it via the account's **long-zero EVM address** (the same address written as the ENS `agent.address`, keeping funding == payer == binding identity). The over-`fundingCap` DENY beat is UNCHANGED and preserved: an over-cap funding transfer from the P-256-owned treasury still returns `FUNDING_DENIED` before broadcast. Privy stays **funding-only**, never a payment co-signer (INVARIANT #6) — the co-signer is a raw Hedera `LEASH_COSIGNER_KEY`, not Privy-held. No new Privy control is added; the reframe does not touch the single-control posture below.
