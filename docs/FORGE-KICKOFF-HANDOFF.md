...
- **PRODUCT SCOPE DECISION 2026-09-12 (Dami): two-path app, see master doc §13.** LEASH is a real multi-tenant product (Privy email/Google login to bring-your-own-org, per-user ENS namespace under `leash.eth`, own Postgres DB of agents, gas-sponsored relayer, register/control) PLUS a zero-setup JUDGE SANDBOX (pre-seeded `acme.leash.eth`, server-side, no login/wallet/ETH, drives the full hero flow). **HARD sequencing rule (master §13.6): core backend + JUDGE SANDBOX FIRST (the scored, must-be-flawless, minimum-eligible bar), then the real multi-tenant path, then best-UI polish. Never let the real-user path endanger the sandbox.** forge must plan WS-5 as WS-5a (sandbox, first) + WS-5b (real console, second), add `db/` + `relayer/`, and carry the §13 deltas into PLAN/ARCHITECTURE/FEATURE-OBSERVABLES.
...
- **[VERIFIED 2026-09-12 — Privy chain 296 support: CONFIRMED] G11 partially resolved.** Live API test with the real app creds: Privy's server-wallet `POST /v1/wallets/{id}/rpc` `eth_sendTransaction` with `caip2:'eip155:296'` IS accepted: Privy routed to a Hedera EVM RPC and ran a transaction simulation (returned Hedera's own `Sender account not found`, i.e. the wallet just needs HBAR funding at WS-0, not an unsupported-chain rejection). Control on Sepolia returned `insufficient funds`. So the "does Privy talk to Hedera EVM" risk is GONE. No `secp256k1Sign` self-broadcast fallback needed for chain support.
- **[VERIFIED 2026-09-12 — Privy policy ENFORCEMENT requires the owner + authorization-signature flow] BUILD-CRITICAL.** A deny-all policy attached via `policy_ids` on a wallet with NO owner did NOT block `eth_sendTransaction` on EITHER chain (both reached broadcast) — Privy enforces wallet policies only through the owner/authorization-key model (docs: `controls/authorization-keys`), where the wallet has a P-256 owner and each RPC request carries a `privy-authorization-signature`. Raw API calls without an owner fail-OPEN. **Implication for the Privy B2B prize (the leaked-key over-fund DENY demo):** the treasury wallet MUST be created with a P-256 owner and driven via the `@privy-io/server-auth` SDK (it computes the authorization signature automatically), NOT raw curl. WS-0 smoke test #1 (do FIRST): generate a P-256 owner key, create an owned wallet + deny/cap policy via the SDK, and confirm an over-cap tx returns a POLICY DENY before broadcast. If that fires, the Privy prize demo is proven. Value-cap conditions compare raw units: pass the cap consistently (hex vs decimal wei mismatch silently mis-evaluated in testing).
