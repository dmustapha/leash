...
## WS-7 build-delta (COMPLETE 2026-09-12)
The WS-7 hardening delta is IMPLEMENTED + gated. Every group re-ran the full regression gate (vm2 6/6, vm1 3/3, `npm run build` PASS, `npm run check` PASS) with no regression to `/demo` or the three prize legs (INVARIANT #10). Baked-in adversarial refinements B-01..B-08 all implemented.

| Item | What landed | Acceptance / evidence |
|------|-------------|-----------------------|
| A1 (S1) | `web/lib/auth.ts` `requireOwner()` (Bearer-only, expiry+app-id audience, ownership JOIN) is the first line of every mutating console route (org/agents/allowlist/reactivate/revoke/pay/fund). Client sends `Authorization: Bearer` via `authedFetch`. | F-016: `web/lib/auth.test.ts` 8/8 incl. the authed-as-A-targets-B IDOR → 403, no-token → 401, wrong-audience → 401. Identity re-derived from token, never client input. |
| A2 (G2) | Org-name collision guard in `/api/org` POST: a label already provisioned by another user → 409 taken-error; never hands the first user's registry to a second user. | F-017. |
| A3 (G3) | `treasury/privy.ts reconcileFundingAllowlist()` read-modify-write UNION into the Privy funding policy on register; asserts `_to in [...]` present + NON-EMPTY (fail-open guard); behind `requireOwner`. | F-018 / B-04. Removes the DEV-030 default-DENY caveat for new console agents. |
| B1 (P1a) | `/api/agents/allowlist` PUT rewrites `allowedPayees` on-chain (setPolicy) + syncs index; UI editor in AgentRow. | F-020, requireOwner-gated. |
| B2 (P1b) | `/api/agents/reactivate` POST re-binds the cleared policy (setPolicy) + status active; UI button on revoked agents. | F-021, requireOwner-gated. |
| B3 (P1c/G4) | `/api/feed` route + `db/index-hcs` filtered queries; org-level `SpendFeed` + per-agent activity drill-down in `/app`; compact live audit scroll in `/demo`. Fixed the indexer mirror query (`gt:0` → no-filter when empty). | F-022: `/api/feed` returns non-empty real ALLOW/DENY indexed from HCS topic 0.0.10496492 (`indexed:true`). |
| C1 (G1) | `scripts/ens/cohold.ts` grants ROLE_SET_TEXT on the agent's `leash.policy` part-resource to the user's Privy embedded wallet via the resolver's scoped `authorizeTextRoles` (raw EAC grantRoles is overridden to revert). New scope-guarded `grant` `RelayOp`. `createOnLogin:'users-without-wallets'`. | F-023 / B-02 / B-03: `agent/cohold.live.ts` 3/3 on-chain — additive (user AND relayer both hold), scope-guard rejects out-of-org grant, confined to the agent's leash.policy key. INVARIANT #8/#11/#12. |
| D1 | `scripts/ens/identity.ts` writes advisory `agent.type`/`agent.description`/`avatar`/optional `erc8004` alongside `leash.policy` on register + seed; surfaced in `/app`, `/demo` (AgentCard), `/proof`. Indexed mirror columns (agent_type/description/avatar). | F-024 / B-07: `facilitator/identity-isolation.integration.ts` module-boundary guard proves the enforcement graph never imports the identity module (INVARIANT #13, advisory-only). Live setText proven on both sandbox agents. |
| A4 (S2) | `web/lib/ratelimit.ts` per-IP token bucket on `/api/demo` + all console mutations → 429 before draining balances. | F-019: `web/lib/ratelimit.test.ts` 3/3. |
| A5 (S3) | `db/replay.ts` + `seen_payments` table; facilitator settle path durable pre-check + persist-before-proceed, FAIL-CLOSED on Neon error (deny, never proceed). Pure gate `authorize.ts` stays DB-free (INVARIANT #3). | F-025 / B-05: `db/replay.integration.ts` round-trip on Neon; INVARIANT #9 durable across restart. |
| E1/E2 | GRANT surface (`/app` register → real mint+setPolicy+identity+cohold txs) + real Privy login surface both render 200. | Interactive OAuth login + on-camera register are Dami's browser steps (demo-rehearsal/demo own recording); surfaces verified ready. |

### WS-7 new risks / notes (for debug)
- **DEV-033**: A5 makes the settle path depend on Neon by design (fail-closed). A Neon outage denies settles (security > availability for the replay guard, per B-05). paymentIds are unique per x402 payload, so durable `seen` never false-REPLAYs a re-run (verified: vm1/vm2 green after A5).
- **DEV-034**: rate-limit + replay stores are in-process/single-instance (correct for single Render instance; best-effort behind multi-instance). Noted in LIMITATIONS.
