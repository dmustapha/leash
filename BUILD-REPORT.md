...
## WS-7 build-delta (pending implementation)
The WS-7 hardening delta (G1-G4 / S1-S3 / B1-B3 controls / C1 co-hold / D1 ENS agent-identity) is SCOPED in `docs/WS7-HARDENING-SCOPE.md` and NOT yet implemented. This report is intentionally incomplete for WS-7: it will get a full WS-7 section (new DEVs, verification, new risks) when the fresh implementation chat completes the delta. Until then, debug should treat WS-7 features (authz ownership JOIN, funding reconcile, allowlist-edit / reactivate, spend feed, co-hold role, ENS identity records, rate limit, durable replay) as spec-only and read `docs/WS7-HARDENING-SCOPE.md` for the acceptance criteria + baked-in adversarial refinements.
