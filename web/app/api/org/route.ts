...
//
// AUTH: the client passes the Privy user id + email from the authenticated session. This route trusts that
// identity only to KEY the multi-tenant index rows (users/orgs); it never gates a spend on it (INVARIANT #3).
//
...
// POST: provision (or reuse) the caller's org. Body: { privyUserId, email?, orgName }.
export async function POST(req: Request) {
  let body: { privyUserId?: string; email?: string | null; orgName?: string };
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: 'bad request body' }, { status: 400 });
  }
  if (!body.privyUserId || !body.orgName) {
    return NextResponse.json({ error: 'privyUserId and orgName are required' }, { status: 400 });
  }

  try {
    const user = await ensureUser(body.privyUserId, body.email ?? null);
    const label = toLabel(body.orgName);

    // Reuse an existing org for this user (idempotent: one org per name).
...
// GET ?privyUserId=... : the caller's org + its agents (the multi-tenant console view). Index reads only.
export async function GET(req: Request) {
  const privyUserId = new URL(req.url).searchParams.get('privyUserId');
