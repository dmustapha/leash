...
| DH-3 | debug | stress_test | P1 | KNOWN-RISKS handoff (DEV-033): A5 durable replay must FAIL-CLOSED on a Neon outage — a store error denies the settle, never proceeds. | stress simulates Neon-down on the settle path and asserts abort=REPLAY (no settle), plus a genuine replay of the same paymentId is rejected across a facilitator restart | DONE (stress: replay.integration live Neon PASS; A5 fail-closed) |
| DH-4 | debug | stress_test | P2 | KNOWN-RISKS handoff (A4/DEV-034): rate limit must 429 a burst before draining fee-payer/agent balance; XFF-spoof is a known single-instance bypass. | stress bursts /api/demo + a console mutation and asserts 429 before balance drain | DONE (stress: ratelimit unit + live burst 8×401→6×429) |
| DH-5 | debug | stress_test | P2 | KNOWN-RISKS handoff (A3/B-04): a SECOND new-agent register must keep the FIRST agent fundable (union preserves existing) and over-fund still DENIES. | stress registers 2 agents and proves both fund in-cap + both over-fund DENY on the real token | DONE (stress: A3 union + 2 live funded registers + C-PRIVY-DENY) |
...
| DH-7 | debug | stress_test | P2 | KNOWN-RISKS handoff (B1/B2/B-08): allowlist-edit + reactivate cross-tenant negatives — A must not edit/reactivate B's agent (requireOwner covers it; confirm end-to-end). | stress asserts 403 on authed-as-A editing/reactivating B's agent, and OFF_ALLOWLIST after an allowlist edit | DONE (stress: live cross-tenant allowlist+reactivate → 403) |
...
### stress_test (complete, 2026-09-12)

#### Done
- **Confidence 92/100 — Battle-tested.** 61 tests, 0 failures, 0 unresolved, 0 fixes needed. STRESS-TEST-PLAN.md + STRESS-TEST-REPORT.md + .stress-test-state.json written.
- Stress-owned handoffs closed: **DH-3** durable replay (`db/replay.integration` live Neon PASS + A5 fail-closed), **DH-4** rate-limit (unit + LIVE burst 8×401→6×429 before expensive work), **DH-5** funding union (A3 + 2 live funded registers + C-PRIVY-DENY), **DH-7** cross-tenant allowlist PUT + reactivate POST → **403** (live). Plus API smoke: malformed→400, missing-auth→401, no-orgId→400.
- Enforcement gate P0 exhaustive: authorize 43 unit + vm2 6/6 + vm3 7-live/1-integ (every DENY reason code). Falsification (8.5): the authorize/vm3 DENY beats ARE feed-a-lie tests (over-cap→assert deny). No ASYNC-TIMEOUT/MISSING-EVENTS/NO-FINAL-STATE.
- Phase 5.5 AI failure modes SKIPPED (no LLM-in-loop agent); per-instance config isolation PASS (DH-7 + distinct per-agent ENS policy).

#### Deferred (honest)
- **RF-4** (env-configurable mirror base for a LIVE BEAT-7 kill beat): NOT implemented — the mirror base is a hardcoded const in the co-sign settle path; making it env-configurable touches the FROZEN /demo settle path, deemed too risky <16h to deadline. Mirror-down fail-closed IS proven at integration tier (vm3 BEAT-7 RPC_ERROR + C-MIRROR-DOWN-DENY). RF-5 therefore proven at integration tier; live on-camera kill-injection deferred. Rolling-WIDTH half already consensus-anchored (debug-cleared).

#### For Next Skill (deploy → livetest)
- No blockers, no unresolved bugs. Honesty locks intact. Deploy: Vercel `leash-ens` + `leash.ink`; Render `leash-facilitator`/`leash-resource`; GUARD .env (R-14, never let Vercel clobber .env.local); Render free tier spins down (DS-5 — keep-warm or plan:starter for durable replay). Frozen /demo behavior NO-TOUCH.
