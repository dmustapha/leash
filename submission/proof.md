...
- **Authed bind e2e (wire RF-1, live Privy JWT):** `POST /api/agents` bind branch → **200** with co-signed KeyList account `0.0.10511140` (long-zero `0x00…a06324`) + on-chain-resolved identity (agentId 7395) + `leash.policy` (`policyTx 0x4c254de90536bd5349a6171a2efb8113e74e7771667f5f41eba4170e18727969`). Authed-as-A targeting another org → **403** (IDOR blocked; `requireOwner` ownership JOIN). Real revoke → index sync proven against live Neon (DH-2).
- **Co-hold kill-switch, persistent (verify_milestone DH-6):** authed `/app`-style register grants `ROLE_SET_TEXT` on the agent's `leash.policy` to the signed-in user alongside the relayer (additive) — persistent Sepolia grant tx `0x31559a9b7300bb5e4eeb8759d7e1285f14b423050ae451eb16f187eab49e0101`, `coholdVerified=true` (both hold the role).
